...
(define-library (voronoi-terrain-main)
  ...
  (begin
    ...
    (define (write-surface-texture model width height output-x-size output-z-size height-function)
      (let* ((output-image-width 1024)
             (output-image-height 1024)
             (octree (model->octree model (model-aa-box model)))
             (img (raster-new output-image-width output-image-height (vector 255 255 255 255)))
             (coords->model-x (lambda (img-x img-y)
                                (/ (* img-x output-x-size) output-image-height)))
             (coords->model-z (lambda (img-x img-y)
                                (/ (* (- output-image-width img-y) output-z-size) output-image-width)))
             (previous-normal #f)
             (previous-face #f)
             )
        (do ((y 0 (+ y 1)))
          ...
          (do ((x 0 (+ x 1)))
            ...
            ;; (cerr "x=" x ", y=" y "\n")
            (let* ((model-x (coords->model-x x y))
                   (model-z (coords->model-z x y))
        ...
        (for-each
         (lambda (node)
           (let* ((data (node-value node))
                  ...
                  (index (model-append-vertex! model (make-vertex-from-point/color point-3d-scaled #f))))
