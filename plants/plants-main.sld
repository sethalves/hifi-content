      ...
      (let* ((args (parse-command-line `((-?) (-h)
        ...
        (let* ((model (make-empty-model))
          ...
          (let* ((material #f)
                 (leaf-half-width 0.3)
                 (p0 (vector 0 0 0))
                 (p1 (vector leaf-half-width 0.8 0.5))
                 (p2 (vector (- leaf-half-width) 0.8 0.5))
                 (p3 (vector 0 1.0 0.5))
                 (p4 (vector 0 0.7 1.0)))
            (define (leaf q)
              (define (q*p v) (vector3-rotate v q))
              (mesh-append-triangle! model mesh material (map q*p (list p0 p1 p2)))
              (mesh-append-triangle! model mesh material (map q*p (list p0 p3 p1)))
              (mesh-append-triangle! model mesh material (map q*p (list p0 p2 p3)))
              (mesh-append-triangle! model mesh material (map q*p (list p1 p3 p4)))
              (mesh-append-triangle! model mesh material (map q*p (list p3 p2 p4)))
              (mesh-append-triangle! model mesh material (map q*p (list p2 p1 p4)))
              )
