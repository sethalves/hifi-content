(define-library (plants-main)
  ...
  (begin
    ...
    (define (make-plant model mesh plant-specification)
      ...
      (let* ((leaf-material
              ;; (model-get-material-by-name model "leaf_mtl")
              #f
              )
             (type (list-ref plant-specification 3))
             (G (vector 0 1 0))
             (W (vector 1 1 1))
             (c0 (case type  ((0) G) ((1) W) ((2) G)))
             (c1 (case type  ((0) G) ((1) G) ((2) W)))
             (c2 (case type  ((0) G) ((1) G) ((2) W)))
             (c3 (case type  ((0) G) ((1) W) ((2) G)))
             (c4 (case type  ((0) G) ((1) G) ((2) W)))
             (leaf-half-width 0.3)
             (leaf-low-point 0.8)
             (leaf-high-point 1.0)
             (p0 (vector 0 0 0 0 1 0))
             (p1 (vector leaf-half-width leaf-low-point 0.5 0 1 0))
             (p2 (vector (- leaf-half-width) leaf-low-point 0.5 0 1 0))
             (p3 (vector 0 leaf-high-point 0.5 0 1 0))
        ...
        (define (leaf q)
          ...
          (mesh-append-triangle! model mesh leaf-material (map transform-point (list p0 p1 p2)) `(,c0 ,c1 ,c2))
          (mesh-append-triangle! model mesh leaf-material (map transform-point (list p0 p3 p1)) `(,c0 ,c3 ,c1))
          (mesh-append-triangle! model mesh leaf-material (map transform-point (list p0 p2 p3)) `(,c0 ,c2 ,c3))
          (mesh-append-triangle! model mesh leaf-material (map transform-point (list p1 p3 leaf-tip)) `(,c1 ,c3 ,c4))
          (mesh-append-triangle! model mesh leaf-material (map transform-point (list p3 p2 leaf-tip)) `(,c3 ,c2 ,c4))
          (mesh-append-triangle! model mesh leaf-material (map transform-point (list p2 p1 leaf-tip)) `(,c2 ,c1 ,c4)))
      ...
      (let* ((args (parse-command-line `((-?) (-h)
        ...
        (let* ((model (make-empty-model))
          ...
          (cond ((not (null? group-input-file-names))
                ...
                (else
                 (make-plant model mesh `((0 0 0) (1 0 0 0) 1 ,type))))
