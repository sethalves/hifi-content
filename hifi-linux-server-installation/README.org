...
The only reason for Ubuntu 17 is to get acceptable Qt packages.  Older Ubuntu works fine
if you can run Qt's installer.  It's possible to run [[http://download.qt.io/official_releases/qt/5.9/][Qt's installer]] on a local Linux machine
and install to ~\~/Qt~, and then
...
you'll need to adjust the path to match the exact version of Qt you install.
