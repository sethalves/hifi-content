...
Create a "droplet".  "Ubuntu 17.04 x64, 2 GB Memory / 40 GB Disk"
...
ExecStartPre=/bin/bash -c 'if /usr/bin/pgrep -l domain-server; then /usr/bin/pkill -SIGKILL -f /etc/hifi/server/active/domain-server; fi'
ExecStart=/bin/bash -c 'ulimit -c unlimited; /home/hifi/hifi/build/domain-server/domain-server 2>&1 >>  /var/log/hifi/domain-server.log || echo $? > /tmp/domain-server.exit'
[Install]
